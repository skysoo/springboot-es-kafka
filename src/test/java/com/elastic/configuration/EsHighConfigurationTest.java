...
import com.elastic.service.HUCController;
...
public class EsHighConfigurationTest {
    ...
    @Autowired
    private HUCController hucController;
    ...
    public void esGetData(){
        hucController.esDocSearch("webtest1","search");
    }
    /**
...
//        }
    ...
    /**
     * Future Task는 get으로 데이터 가져오지만 Completable Future는 get할 필요가 없다.
     **/
    // TODO: 2020-01-22 Completable Future Task 구현
    ...
    public void indexNoiseDataTest(IndexRequest indexRequest) {
        try {
            ...
            StopWatch stopWatch = new StopWatch();
            stopWatch.start();
            log.info("##### "+indexRequest.getDescription());
            for (int i = 0; i < 70; i++) {
                IndexResponse indexResponse = restHighLevelClient.index(indexRequest, RequestOptions.DEFAULT);
                if (indexResponse.getShardInfo().getFailed() > 0) {
                    log.error("##### Index response is failed {}", indexName);
