...
public class EsLowController {
    ...
    public EsLowController(EsLowConfiguration esLowConfiguration, Properties properties, NoiseDataManager noiseDataManager) {
    ...
    public void esClusterStat(@RequestParam String indexName,@RequestParam String method) {
        if(esLowConfiguration.get(urlStr()+"_cluster/stats",method))
    ...
    public void esIndexDocCount(@RequestParam String indexName,@RequestParam String method) {
        if(esLowConfiguration.get(urlStr()+indexName+"/_count",method))
    ...
    public void esDocSearch(@RequestParam String indexName,@RequestParam String method){
       if(esLowConfiguration.get(urlStr()+indexName+"/_search?pretty",method))
    ...
    public void esIndexSave(@RequestParam String indexName){
        ...
        if(esLowConfiguration.post(urlStr()+indexName+"/_doc?pretty",stringJson)) {
    ...
    public void esIndexCreate(@RequestParam String indexName,@RequestParam int shardNum,@RequestParam int replicaNum) {
        if(esLowConfiguration.put(urlStr()+ indexName + "?pretty", "{\n" +
                "    \"settings\" : {\n" +
                "        \"index\" : {\n" +
    ...
    public void esDeleteIndex(@RequestParam String indexName){
        if(esLowConfiguration.delete(urlStr()+indexName))
